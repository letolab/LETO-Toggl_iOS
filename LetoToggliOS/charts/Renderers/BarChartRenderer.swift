        ...
        for j in 0 ..< Int(ceil(CGFloat(dataSet.entryCount) * animator.phaseX))
        {
            ...
            else
            {
                ...
                for k in 0 ..< vals!.count
                {
                    ...
                    // Set the color for the currently drawn value. If the index is out of bounds, reuse colors.
                    if dataSet.useStackedColors {
                        var color = NSUIColor.blackColor().CGColor
                        if dataSet.stackedColors.count > j && dataSet.stackedColors[j].count > k {
                            color = dataSet.stackedColors[j][k].CGColor
                        }
                        CGContextSetFillColorWithColor(context, color)
                    }else{
                        CGContextSetFillColorWithColor(context, dataSet.colorAt(k).CGColor)
                    }
                    if k ==  vals!.count - 1  && value >= 0.0{
                        // Rounded top
                        barRect.size.height += 5
                        let cornerRadius: CGFloat = CGRectGetWidth(barRect) <= 5 ? 2.0 : 4.0
                        let bezierPath = UIBezierPath(roundedRect: barRect, byRoundingCorners: [UIRectCorner.TopLeft, UIRectCorner.TopRight], cornerRadii: CGSizeMake(cornerRadius, cornerRadius))
